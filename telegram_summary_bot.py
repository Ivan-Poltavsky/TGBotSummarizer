...
async def start(update: Update, context: CallbackContext):
    ...
    CREATE TABLE IF NOT EXISTS books (
        id TEXT NOT NULL,
        author_and_title TEXT NOT NULL,
...
async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        ...
        "/help - Показать информацию о доступных командах\n"
        "/get_summary - Получить описание книги по ID\n"
        "/all_list_ID - Получить полный список ID\n"
        "/stop - Прекратить взаимодействие с ботом\n"
    )
...
async def process_document(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    author_and_title = gpt(book_summary[:50], 'body1.json')
    book_id = uuid.uuid4()
    conn = context.user_data.get('db_connection')
    cursor = context.user_data.get('db_cursor')
    if conn and cursor:
        insert_sql = '''
        INSERT INTO books (id, author_and_title, summary)
        VALUES (?, ?, ?)
        '''
        cursor.execute(insert_sql, (str(book_id), author_and_title, book_summary))
        conn.commit()
...
async def get_summary(update: Update, context: CallbackContext):
    ...
    await send_long_message(context.bot, update.message.chat_id, summary)

@check_stopped
async def all_list_ID(update: Update, context: CallbackContext):
    conn = context.user_data.get('db_connection')
    cursor = context.user_data.get('db_cursor')
    select_sql = '''
    ...
    if row_count != 0:
        select_sql = '''
            SELECT id, author_and_title FROM books
            '''
        cursor.execute(select_sql)
        result_select = cursor.fetchall()
        message = "Список ID c информацией о книге:\n"
        for id, author_title in result_select:
            message += f"ID: {id}\n Информация о книге:\n {author_title}\n\n"
        await send_long_message(context.bot, update.message.chat_id, message)
    else:
        await update.message.reply_text("Ни одна книжка еще не была добавлена в базу данных")
...
def main():
    ...
    application.add_handler(CommandHandler('all_list_ID', all_list_ID))
